/*
  ...
  VARIANT (2026-08-14): Improvement-SmallTarget. Forked from two-prong to test
  visual target-circle size as a manipulated variable, holding the scoring
  zones (SUCCESS_ZONE_SIZES) fixed. The rendered target ring's radius is
  reduced 80% (1.2 -> 0.24, see drawScene()) versus two-prong/
  Improvement-LargeTarget — purely cosmetic, the coercion engine and scoring
  zones are untouched, so a "success" still lands within the same 1.2/0.9/
  0.6/0.3 zones as before; it just looks smaller relative to the visual ring.
  Locked to the improvement condition only (see CONDITIONS below).
  ==========================================================================
*/
...
const BT_CONFIG = {
  ...
  // Improvement-LargeTarget. Update before running real participants if that's
  // not desired.
...
function drawScene(ctx){
  ...
  // solid white target ring, flat on the ground at RING_D. Radius reduced 80%
  // from two-prong's 1.2 (cosmetic only — SUCCESS_ZONE_SIZES/scoring zones are
  // unchanged, see VARIANT note at top of file).
  drawGroundRing(ctx, 0, RING_D, 0.24, false);
}
...
async function startExperiment(){
  ...
  // NOTE: set for local testing of this variant — remember to null this out
  // before running real participants (see project_force_condition_bug memory
  // for why a stuck FORCE_CONDITION is dangerous).
  const FORCE_CONDITION = "improvement";
  if(FORCE_CONDITION){
